...
static inline void transpose_pack8_interleave4(
    ...
    for (int iter = 0; iter < howmany; ++iter, data += howmany_stride) {
        #pragma unroll
        for (int i = 0; i < 8; ++i) {
            buffer[i * (warpSize + 1) + lane_id] = data[i * stride];
        }

        #pragma unroll
        for (int i = 0; i < 8; ++i) {
            data[i * stride] = buffer[(lane_id % 8) * (warpSize + 1) + (lane_id & -8) + i];
        }
    }
}
...
static void bm3d(
    ...
    int x = (warpSize / 8 * blockIdx.x + lane_id / 8) * block_step;
                ...
                for (int i = 0; i < ps_num; ++i) {
                    ...
                    for (int row_i = top; row_i <= bottom; ++row_i) {
                        ...
                        for (int col_i = left; col_i <= right; ++col_i) {
                            ...
                            float pre_error = __shfl_up(frame_errors8, 1, 8);
                            int pre_index_x = __shfl_up(frame_index8_x, 1, 8);
                            int pre_index_y = __shfl_up(frame_index8_y, 1, 8);
...
const char * launch(
    ...
    checkError(hipMemcpy2DAsync(d_src, pitch, h_res, pitch, width * sizeof(float), (final_ ? 2 : 1) * num_planes * temporal_width * height, hipMemcpyHostToDevice, stream));

    checkError(hipMemset2DAsync(d_res, pitch, 0, width * sizeof(float), num_planes * temporal_width * 2 * height, stream));
    ...
    auto gridDim = dim3(
        (width + ((warp_size / 8) * block_step - 1)) / ((warp_size / 8) * block_step),
        (height + (block_step - 1)) / block_step
    );
    auto blockDim = dim3(warp_size);
    void * kernelArgs[] {
        &d_res, &d_src,
        &width, &height, &stride,
    ...
    checkError(hipLaunchKernel(func, gridDim, blockDim, &kernelArgs[0], 8 * (warp_size + 1) * sizeof(float), stream));

    checkError(hipMemcpy2DAsync(h_res, pitch, d_res, pitch, width * sizeof(float), num_planes * temporal_width * 2 * height, hipMemcpyHostToDevice, stream));
